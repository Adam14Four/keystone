...
function render(req, res, view, ext) {
	...
	var filterNav = function (keystoneNav, user) {
		var cloneNav = lodash.cloneDeep(keystoneNav);
		var removeSections = [];

		_.each(cloneNav.sections, function (section, idx) {
			var removeIdxs = [];
			
			_.each(section.lists, function (secList, lidx) {
				if (req.user && !keystone.list(secList.key).roleAllowed(req.user.role)) {
					removeIdxs.unshift(lidx);
				}
			});
			
			_.each(removeIdxs, function (lidx) {
				cloneNav.sections[idx].lists.splice(lidx, 1);
			});

			if (section.lists.length === 0) {
				removeSections.unshift(idx);
			}
		});
		
		_.each(removeSections, function (idx) {
			cloneNav.sections.splice(idx, 1);
		});

		return cloneNav;
	};

	var filterSectionNav = function (sectionNav, user) {
		var cloneNav = lodash.cloneDeep(sectionNav);
		var removeIdxs = [];

		_.each(cloneNav.lists, function (secList, lidx) {
			if (req.user && !keystone.list(secList.key).roleAllowed(req.user.role)) {
				removeIdxs.unshift(lidx);
			}
		});
		...
		_.each(removeIdxs, function (lidx) {
			cloneNav.lists.splice(lidx, 1);
		});

		return cloneNav;
	};

	var nav = filterNav(keystone.nav, res.user);
	var section = {};

	if (ext.section && ext.section.lists) {
		section = filterSectionNav(ext.section, res.user);
