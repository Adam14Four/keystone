...
function routes(app) {
	...
	function initList(respectHiddenOption, skipUserCheckOnGet) {
		return function(req, res, next) {
			req.list = keystone.list(req.params.list);

			var userAllowed = (req.user && req.list && req.list.roleAllowed(req.user.role)) || (skipUserCheckOnGet && req.method === 'GET');

			if (!req.list || (respectHiddenOption && req.list.get('hidden')) || !userAllowed) {
	...
	// Generic Lists API
	app.all('/keystone/api/:list/:action(autocomplete|order|create|fetch)', initList(false, true), require('../../admin/api/list'));
	...
	app.get('/keystone/api/:list/:id', initList(false, true), require('../../admin/api/item/get'));
